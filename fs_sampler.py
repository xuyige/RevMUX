...
class FewShotSampler(object):
    ...
    def __init__(self,
        ...
        self.name_of_label_column = name_of_label_column

    def __call__(self,
                 train_dataset: Union[DataSet, Dataset, List],
                 valid_dataset: Optional[Union[DataSet, Dataset, List]] = None,
                 seed: Optional[int] = None
                ) -> Union[DataSet, Dataset, List]:
        '''
    ...
    def _sample(self,
                data: Union[DataSet, Dataset, List],
                seed: Optional[int],
                sample_twice=False,
                ) -> Union[DataSet, Dataset, List]:
        if seed is not None:
        ...
        if self.num_examples_per_label is not None:
            assert hasattr(data[0], self.name_of_label_column) or (data[0][self.name_of_label_column] is not None), \
                f"sample by label requires the data has a '{self.name_of_label_column}' attribute."
            if hasattr(data[0], self.name_of_label_column):
                labels = [getattr(x, self.name_of_label_column) for x in data]
            else:
                labels = [x[self.name_of_label_column] for x in data]
        ...
        if sample_twice:
            ...
            if self.num_examples_per_label_dev is not None:
                assert hasattr(data[0], self.name_of_label_column) or (data[0][self.name_of_label_column] is not None), \
                    f"sample by label requires the data has a '{self.name_of_label_column}' attribute."
                if hasattr(data[0], self.name_of_label_column):
                    remain_labels = [getattr(x, self.name_of_label_column) for x in data]
                else:
                    remain_labels = [x[self.name_of_label_column] for x in data]
            ...
            if isinstance(data, Dataset) or isinstance(data, DataSet):
