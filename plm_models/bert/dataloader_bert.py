...
class BaseTCLoader(Loader):
    ...
    def pre_process(self, data_set: DataSet) -> DataSet:
        ...
        for example in data_set:
            ...
            target_encodings = self.tokenizer.encode_plus(example['ground_truth'])
            ...
            encodings = {
                ...
                'decoder_input_ids': target_encodings['input_ids'][: 10],
                'decoder_attention_mask': target_encodings['attention_mask'][: PLM_MAX_LENGTH],
...
class NLILoader(BaseTCLoader):
    ...
    def convert_examples(self, example):
        if self.n_prompt_tokens > 0:  # use randomly selected words as initial prompt
            offset = 105
            # if 'MRPC' in self.__class__.__name__ and not self.set_offset:
            #     offset = 1000
            #     self.set_offset = True
            #     logger.info(f'Set offset to {offset}.')
            prompt = self.tokenizer.decode(list(range(offset, offset + self.n_prompt_tokens)))
            example['input_text'] = (f'{self.tokenizer.mask_token} '
                                     f'{prompt} . {example["sentence1"]} '
                                     f'{self.tokenizer.sep_token} {example["sentence2"]} .')
            example['ground_truth'] = self.label2text[example['labels']]
        else:
            example['input_text'] = (f'{self.tokenizer.mask_token} . {example["sentence1"]} '
                                     f'{self.tokenizer.sep_token} {example["sentence2"]} .')
            example['ground_truth'] = self.label2text[example['labels']]
...
class SST2Loader(BaseTCLoader):
    ...
    def convert_examples(self, example):
        if self.n_prompt_tokens > 0:  # use randomly selected words as initial prompt
            offset = 1000
            prompt = self.tokenizer.decode(list(range(offset, offset + self.n_prompt_tokens)))
            example['input_text'] = f'{self.tokenizer.mask_token} {prompt} . {example["sentence"]} .'
            example['ground_truth'] = self.label2text[example['labels']]
        else:
            example['input_text'] = f'{self.tokenizer.mask_token} . {example["sentence"]} .'
            example['ground_truth'] = self.label2text[example['labels']]
        return example
