...
from revmux.plm_models.t5.modeling_t5 import T5ForConditionalGeneration
...
class VanillaAdapterBatchInference(nn.Module):
    ...
    def __init__(self, model_name, target_index: List[int], n_prompt_tokens=50,
        ...
        logger.info(f'Compute Cosine Similarity: {self.compute_cos_similarity}')
        logger.info(f'Use AdaPrompt: {self.use_ada_prompt}')
    ...
    def compute_one_division(
        ...
        logits_mask = torch.zeros_like(student_logits) - 1e5
        ...
        if self.training:
            ce_loss = self.ce_loss_func(student_logits[:, 1], target_ids[:, 1])
            ...
            normalize_student_sequence_output = F.normalize(student_sequence_output[:, 1], dim=1)
            normalize_teacher_sequence_output = F.normalize(teacher_sequence_output[:, 1], dim=1)
...
class ReversibleBatchInference(nn.Module):
    ...
    def __init__(self, model_name, target_index: List[int], n_prompt_tokens=50,
                 ...
                 use_ada_prompt=False, invertible_decompose=False):
        super().__init__()
        self.compose_size = compose_size
        ...
        self.compute_cos_similarity = compute_similarity
        logger.info(f'Compute Cosine Similarity: {self.compute_cos_similarity}')
        logger.info(f'Invertible Decompose: {self.invertible_decompose}')
        logger.info(f'Use AdaPrompt: {self.use_ada_prompt}')
    ...
    def compute_invertible_representations(self, new_input_embeds_with_prompt):
        for idx in range(self.compose_size):  # idx = 1
            idx1 = self.compose_size - idx - 2
            idx2 = self.compose_size - idx - 1
            new_input_embeds_with_prompt[:, idx1] = (new_input_embeds_with_prompt[:, idx1] +
                                                     self.f_adapter[idx](new_input_embeds_with_prompt[:, idx2]))
    ...
    def compute_one_division(
        ...
        if self.training:
            ce_loss = self.ce_loss_func(student_logits[:, self.ans_pos], target_ids[:, self.ans_pos])
